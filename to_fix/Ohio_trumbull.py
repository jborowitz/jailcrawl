...
ROW_INDEX = 771 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        view_current = browser.find_element_by_xpath('//*[@id="btnCurrent"]')
        view_current.click()
        ...
        while finished == False:
            ...
            dropdown = browser.find_element_by_xpath('//*[@id="selGoTo"]')
            dropdown.click()
            logger.info('clicking goto dropdown')
            dropstring = str(page_index*50+1)+'_'
            logger.info('sending page number _%s_',dropstring)
            dropdown.send_keys(dropstring)
            ...
            if len(pages) > 1 and store_source in pages:
                finished = True
                try:
                    dropdown = browser.find_element_by_xpath('//*[@id="selGoTo"]')
