...
ROW_INDEX = 80 # Change this for each scraper
def main(roster_row):
    try:
        ...
        while not finished:
            try:
                ...
                page_index += 1
                nextpage = browser.find_element_by_xpath('//*[@id="cphBody_cphCenter_ctl01_paginationtop_paginationtop_lb{}"]'.format(str(page_index)))
                nextpage.click()
                time.sleep(np.random.uniform(5,10,1))
