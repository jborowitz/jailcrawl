...
def main(roster_row):
    try:
        ...
        # Click Page Size -> 30
        elem = browser.find_element_by_xpath('//*[@id="ContentPlaceHolder1_ddlPageSize"]')
        elem.click()    
        time.sleep(np.random.uniform(1,3,1))
        elem = browser.find_element_by_xpath('//*[@id="ContentPlaceHolder1_ddlPageSize"]/option[3]')
        elem.click()    
        time.sleep(np.random.uniform(1,3,1))
        ...
        page=0
        for link in soup.findAll("span", {"id":"lblInmateslisted"}):
            page=str(link.text)
            page=re.sub("30 inmates shown out of ", "", page)
            page=page[:page.index(" total")]
            page=int(page)
            page=int(page/30)+(page % 30 > 0)

        #Crawling through all the pages
        string = str(1)
        for i in range(2,page+1):
            elem = browser.find_element_by_xpath('//*[@id="ContentPlaceHolder1_cmdNext2"]')
            elem.click()        
            time.sleep(np.random.uniform(7,10,1))
            store_source = browser.page_source
            string=str(i)
