...
ROW_INDEX = 572 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        """
        Supplied link is only for the week of August 27-Sept 02, 2019.
        
        Original link: http://www.wcsdmo.net/pdfs/35%20-%20Weekly%20Arrest%20Report%2027AUG2017%20thru%202SEP2017.pdf
        Source: http://www.wcsdmo.net/jail.html
        ...
        urlAddress2 = urlAddress.replace('jail.html', '')
        #Find pdf links:
        soup = BeautifulSoup(store_source, 'lxml')
        linkbox = soup.find_all("ul")
        pdf_links = linkbox[0].find_all('a', href=re.compile("pdf"))

        suffixes = []
        pages = []
        
        for pdf_link in pdf_links:
            suffixes.append(pdf_link['href'].replace(' ', '%20'))
        ...
        for suffix in suffixes:
            ...
            #Get pdf content
            req = requests.get(urlAddress2+suffix)
