...
ROW_INDEX = 814 # Change this for each scraper. This references the row
# of the main jailcrawl spreadsheet. This index will be used to look up
# the URL as well as state/county info
THIS_STATE = 'oregon' # Change the current state/county information. 
THIS_COUNTY = 'multnomah'
def main(roster_row):
    try:
        ...
        for letter in letters:
            #Since there is no back button once a search has been called,
            #get the home page every time.
            browser.get(urlAddress) 
            
            #Wait
            time.sleep(np.random.uniform(5,10,1))
            
            #Find field for last name, enter letter
            lastname = browser.find_element_by_xpath('//*[@id="LastName"]')
            lastname.send_keys(letter)
            logger.info('Searching for lastname _%s_', letter)
            
            #Click button to search for letter
            search = browser.find_element_by_xpath('//*[@id="search-bookings-form"]/p/input')
            search.click()
            ...
            pages[letter] = store_source
            logger.info('storing page for _%s_', letter)
