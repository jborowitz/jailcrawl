...
ROW_INDEX = 247 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        ##### Extract the PDF - We will use requests #####
        ...
        website_df=website_df[website_df.link.str.contains("/docview.aspx?")]
        
        # Resetting index
        website_df = website_df.reset_index()
        website_df = website_df.drop(columns=['index'])
        
        # Pulling the first link
        last_link = str(website_df.link.iloc[0])
                
        ##### Extract the PDF -> We will use requests instead of selenium #####
        r = requests.get("https://www.worthcounty.org/"+last_link)
        store_source = r.content
