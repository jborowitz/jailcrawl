...
#def main(urlAddress):
    #try:
        ...
        #text=str(soup)
        #text=text[text.index("Number of Inmates:"):text.index(' )')]
        #text=re.sub('Number of Inmates: ',"", text)
        #page=int(text)/10
        #page=math.ceil(page)
        ...
        #for i in range(2,page+1):
            #elem = browser.find_element_by_xpath('//*[@id="wpv-view-layout-285"]/div[31]/div/ul/li['+str(i)+']/a')
            #elem.click()        
            #time.sleep(np.random.uniform(3,5,1))
            #store_source = browser.page_source
            #string=str(i)
            #file_ = open(root_directory +'/scraper_test_update/' +state+"/"+"_"+ county +"_"+ date_test +"_"+ string+ '.html', 'w', encoding='utf-8')
