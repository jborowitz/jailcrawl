...
ROW_INDEX = 725 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        pdf_link = soup.find_all('a', text=re.compile(".*Updated 12:00 a.m. Daily.*"))
        new_url = pdf_link[0]['href']
        
        req2 = requests.get(new_url)
        xlsx_data = req2.content
        ## Code to save a page and log appropriately
        save_to_s3(store_source, page_index, roster_row, filetype='xls')
