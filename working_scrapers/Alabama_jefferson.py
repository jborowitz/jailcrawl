...
ROW_INDEX = 14 # Change this for each scraper
def main(roster_row):
    try:
        """
        Old URL was for wrong state
        
        OLD URL: https://www.co.jefferson.tx.us/Sheriff/content/documents/inmate/CURRENTINMATES.PDF
        NEW URL: http://sheriff.jccal.org/NewWorld.InmateInquiry/AL0010000
        ...
        assert roster_row['County'].lower() == 'jefferson'

        time.sleep(np.random.uniform(5,10,1))
        menu = browser.find_element_by_xpath('//*[@id="uxFacility"]')
        menu.send_keys('Jeff')
        
        search = browser.find_element_by_xpath('//*[@id="Inmate_Index"]/div[1]/form/div/div/input[1]')
        search.click()
        ...
        while not finished:
            time.sleep(np.random.uniform(5,10,1))
            next_page = browser.find_element_by_xpath('//*[@id="Inmate_Index"]/div[2]/div[3]/a[3]')
            next_page.click()
            store_source = browser.page_source
