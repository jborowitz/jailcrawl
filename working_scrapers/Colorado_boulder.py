...
ROW_INDEX = 78 # Change this for each scraper
def main(roster_row):
    try:
        ...
        assert roster_row['County'].lower() == 'boulder'
        pages = []
        ...
        dropdown = browser.find_element_by_xpath('//*[@id="grid"]/div[2]/span[1]/span/span/span[2]/span')
        dropdown.click()
                
        hundred = browser.find_element_by_xpath('//*[@id="grid"]/div[2]/span[1]/span')
        hundred.send_keys(Keys.DOWN)
        hundred.send_keys(Keys.DOWN)
        hundred.send_keys(Keys.RETURN)
        ...
        while not finished:
            try:
                nextpage = browser.find_element_by_xpath('//*[@id="grid"]/div[2]/a[3]')
                nextpage.click()
                time.sleep(np.random.uniform(5,10,1))
                store_source = browser.page_source
                page_index += 1
