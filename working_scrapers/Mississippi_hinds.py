...
ROW_INDEX = 495 # Change this for each scraper. This references the row
...
#        elem.click()
        ...
        while finished == False:
            ...
            #
            print(browser.find_element_by_xpath('//*[@id="printReady"]/div[2]/h3').text)
            currentpage = int((soup.find('h3').text).split(' of ')[0].replace('Page ', ''))
            lastpage = int((soup.find('h3').text).split(' of ')[1])
        
            #Page 1 has only a "Page 2" button, every subsequent page has
            #both "Page X-1" and Page X+1"
            #Check position in page index:
            
            if currentpage == 1:
                nextpage = browser.find_element_by_xpath('//*[@id="printReady"]/form/div/input[5]')
                nextpage.click()
            elif currentpage == lastpage:
                finished = True
            else:
                nextpage = browser.find_element_by_xpath('//*[@id="printReady"]/form/div/input[6]')
                nextpage.click()
