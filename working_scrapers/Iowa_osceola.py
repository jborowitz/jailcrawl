...
ROW_INDEX = 239 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        website_df=website_df[website_df.link.str.contains(".pdf")]
        
        # Resetting index
        website_df = website_df.reset_index()
        website_df = website_df.drop(columns=['index'])
        
        # Pulling the last link
        last = len(website_df)
        last_link = str(website_df.link.iloc[last-1])
