...
ROW_INDEX = 163 # Change this for each scraper. This references the row
# of the main jailcrawl spreadsheet. This index will be used to look up
# the URL as well as state/county info
THIS_STATE = 'hawai' # Change the current state/county information. 
THIS_COUNTY = 'honolulu'
def main(roster_row):
    try:
        ...
        website_df=website_df[website_df.link.str.contains("arrestLogs/current/")]
        ...
        r = requests.get('http://www.honolulupd.org/information/'+website_df["link"].iloc[0]) 
        time.sleep(np.random.uniform(5,7,1))
