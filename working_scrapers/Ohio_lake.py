...
ROW_INDEX = 752 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        "One more step
        Please complete the security check to access www.lakecountyohio.gov"
         
        "I am not a robot" Captcha
        
        Formatting the url for the current day's pdf roster works, though:
        ...
        try:
            pdf_url = 'https://webapps.lakecountyohio.gov/JailRoster/JailRosterFiles/{}_PICTURE.PDF'.format(today)
            req = requests.get(pdf_url)
            req.raise_for_status()
            pdf_data = req.content
            page_index = 'current_day'
        except:
            current_day = False
            
        if current_day == False:
            pdf_url = 'https://webapps.lakecountyohio.gov/JailRoster/JailRosterFiles/{}_PICTURE.PDF'.format(yesterday)
            req = requests.get(pdf_url)
            req.raise_for_status()
            pdf_data = req.content
            page_index = 'previous_day'
