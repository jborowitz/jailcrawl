...
ROW_INDEX = 817 # Change this for each scraper. This references the row
...
THIS_COUNTY = 'tillamook'
def main(roster_row):
    try:
        """
        IFRAME SITE
        
        OLD URL: http://www.co.tillamook.or.us/gov/Jail/Division-inmateList.html
        UPDATED URL: https://www.co.tillamook.or.us/gov/jail/inmatelist/inmate.htm
        ...
        dropdown = browser.find_element_by_xpath('//*[@id="inmateList_length"]/label/select')
        dropdown.send_keys('100')
        logger.info('selecting 100 per page')
    	    
        #Extract the HTML
        store_source = browser.page_source
        pages.append(store_source)
        logger.info('Storing page _%s_', len(pages))
        
        finished = False
        while finished == False:
            
            time.sleep(np.random.uniform(5,10,1))
            nextpage = browser.find_element_by_xpath('//*[@id="inmateList_next"]')
            nextpage.click()
