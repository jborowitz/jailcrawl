...
def main(roster_row):
    try:
        ...
        # Click all inmates (this takes quite some time)
        elem = browser.find_element_by_xpath('//*[@id="rdoAll"]')
        elem.click()    
        time.sleep(np.random.uniform(25,30,1))
        
        # Click to show 100
        elem = browser.find_element_by_xpath('//*[@id="tblInmates_length"]/label/select')
        elem.click()    
        time.sleep(np.random.uniform(1,3,1))
        
        elem = browser.find_element_by_xpath('//*[@id="tblInmates_length"]/label/select/option[4]')
        elem.click()    
        time.sleep(np.random.uniform(1,3,1))
        ...
        page=0
        for link in soup.findAll("div", {"id":"tblInmates_info"}):
            page=str(link.text)
            page=re.sub("Showing 1 to 100 of ", "", page)
            page=page[:page.index(" entries")]
            page=int(page)
        ...
        for i in range(2,page+1):
            elem = browser.find_element_by_xpath('//*[@id="tblInmates_next"]')
            elem.click()        
