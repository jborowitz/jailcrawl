...
ROW_INDEX = 603 # Change this for each scraper. This references the row
# of the main jailcrawl spreadsheet. This index will be used to look up
# the URL as well as state/county info
THIS_STATE = 'nevada' # Change the current state/county information. 
...
def main(roster_row):
    try:
        ...
        elem = browser.find_element_by_xpath('//*[@id="ctl07"]')
        elem.click()
        
        #Wait
        time.sleep(np.random.uniform(5,10,1))
        
        search = browser.find_element_by_xpath('//*[@id="txtSearchName"]')
