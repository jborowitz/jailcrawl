...
ROW_INDEX = 579 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        jailroster = soup.find_all('ol', {'class':'semanticList bullets'})
        prefix = 'https://adlc.us/'
        suffix = jailroster[0].find('a', href=True)['href']
        
        #Wait
        time.sleep(np.random.uniform(5,10,1))
        
        req2 = requests.get(prefix+suffix)
        store_source = req2.content
