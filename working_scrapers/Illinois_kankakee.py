...
ROW_INDEX = 177 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        #Clicking in Custody
        elem = browser.find_element_by_xpath('//*[@id="uxInCustody"]')
        elem.click()        
        
        #Clearing booking date
        elem = browser.find_element_by_xpath('//*[@id="uxBookingFromDate"]')
        elem.click()       
        elem.clear()
        elem = browser.find_element_by_xpath('//*[@id="uxBookingToDate"]')
        elem.click()       
        elem.clear()
        
        #Clicking Search
        elem = browser.find_element_by_xpath('//*[@id="Inmate_Index"]/div[1]/form/div/div/input[1]')
        elem.click()        
        ...
        page=0
        for link in soup.findAll("select", {"name":"Page"}):
            page=str(link.text)
            page=re.sub("\n", " ", page)
            page=page.split()
            a=len(page)
        ...
        for i in range(2, page+1):
            try:
                elem = browser.find_element_by_xpath('//*[@id="Inmate_Index"]/div[2]/div[3]/a[3]')
                elem.click()        
                time.sleep(np.random.uniform(5,7,1))
                store_source = browser.page_source
                string=str(i)
