...
ROW_INDEX = 820 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        # rowser = get_browser() # Get a standard browser
        ...
        for letter in letters:
            req = requests.get(urlAddress.replace('?alpha=A', '?alpha={}'.format(letter)))
            store_source = req.content
            pages[letter] = store_source
            logger.info('got page _%s_', letter)
