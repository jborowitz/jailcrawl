...
ROW_INDEX = 231 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        time.sleep(np.random.uniform(12,15,1))
        ...
        for link in soup.findAll("div", {"class":"record-count-details lbl-med ng-binding"}):
            page=str(link.text)
            page=page[page.index("of"):page.index("results")]
            page=re.sub('of ', "", page)
            page=re.sub(' ', "", page)
            page=int(page)/50
            page=math.ceil(page)
        ...
        for i in range(2,page+1):
            elem = browser.find_element_by_xpath('//*[@id="primary-container"]/div/div/div/zt-collectionview/div[3]/div/div/div/button[2]')
            elem.click()        
            time.sleep(np.random.uniform(12,15,1))
            store_source = browser.page_source
            string=str(i)
