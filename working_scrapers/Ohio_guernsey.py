...
ROW_INDEX = 743 # Change this for each scraper. This references the row
...
THIS_COUNTY = 'guernsey'
def main(roster_row):
    try:
        """
        OLD URL: http://inmateportal.guernseysheriff.com/guernseypublicview/(S(k2t42n3cug0krlujit4h03bw))/BookingFind.aspx
        UPDATED URL: http://inmateportal.guernseysheriff.com/guernseypublicview/BookingFind.aspx
        """
        ...
        for letter in letters:
            ...
            search_field = browser.find_element_by_xpath('//*[@id="InmateLast"]')
            search_field.send_keys(Keys.BACKSPACE, Keys.DELETE)
            search_field.send_keys(letter)
            
            #Locate "Search" button
            time.sleep(np.random.uniform(1,3,1))
            search_button = browser.find_element_by_xpath('//*[@id="btnFind"]')
            search_button.click()
