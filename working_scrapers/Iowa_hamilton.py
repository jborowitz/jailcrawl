...
ROW_INDEX = 235 # Change this for each scraper. This references the row
...
def main(roster_row):
    try:
        ...
        text=str(soup)
        try:
            text=text[text.index("Individuals may leave voicemails for the inmates 24 hours 7 day a week at 515-832-1258."):]
            text=text[text.index("a href="):text.index(' target="_blank"')]
            text=re.sub('"',"", text)
        ...
        #Directly pull the webpage with current inmate information
        browser.get("https://www.hamiltoncounty.org/"+text)
        
        #Extract the HTML from the current inmate page
        store_source = browser.page_source
